---
mode: agent
description: Dodaj nową akcję (np. edycja zadania, sortowanie) do aplikacji w stylu projektu.
---

# Nowa akcja w Task Managerze

Dodaj nową funkcjonalność do aplikacji. Cała logika idzie do `js/app.js`, ewentualne style do `css/styles.css`, ewentualne nowe elementy DOM do `index.html`.

## Wejście

Zapytaj użytkownika (jeśli nie podał):

- **Nazwa akcji** (np. „edycja tytułu zadania", „sortowanie po dacie")
- **Trigger** (klik przycisku, zmiana selecta, submit formularza)
- **Walidacja** (jeśli dotyczy — jakie reguły)
- **Co się zmienia w stanie `tasks`** (mutacja jakich pól)
- **Co się zmienia w UI** (jak ma wyglądać po akcji)

## Co masz zrobić

1. Otwórz `js/app.js` i znajdź miejsce na nową funkcję (sekcja akcji, między istniejącymi `addTask` / `toggleTask` / `deleteTask`).
2. Dodaj funkcję akcji. Zmień stan w zmiennej `tasks`, zapisz do `localStorage`, wywołaj `render()`.
